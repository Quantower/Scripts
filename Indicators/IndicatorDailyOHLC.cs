...
namespace ChanneIsIndicators
{
    public class IndicatorDailyOHLC : Indicator
    {
        ...
        protected override void OnInit()
        {
            ...
            if (this.CurrentChart != null)
                this.CurrentChart.OnUpdateChartSettings += this.CurrentChart_OnUpdateChartSettings;
        ...
        public override IList<SettingItem> Settings
        {
            ...
            set
            {
                if (value.GetItemByName("OpenLineOptions")?.Value is LineOptions openOptions)
                    this.OpenLineOptions = openOptions;
                if (value.GetItemByName("OpenExtendLineOptions")?.Value is LineOptions openExtendOptions)
                    this.OpenExtendLineOptions = openExtendOptions;

                if (value.GetItemByName("HighLineOptions")?.Value is LineOptions highOptions)
                    this.HighLineOptions = highOptions;
                if (value.GetItemByName("HighExtendLineOptions")?.Value is LineOptions highExtendOptions)
                    this.HighExtendLineOptions = highExtendOptions;

                if (value.GetItemByName("LowLineOptions")?.Value is LineOptions lowOptions)
                    this.LowLineOptions = lowOptions;
                if (value.GetItemByName("LowExtendLineOptions")?.Value is LineOptions lowExtendOptions)
                    this.LowExtendLineOptions = lowExtendOptions;

                if (value.GetItemByName("CloseLineOptions")?.Value is LineOptions closeOptions)
                    this.CloseLineOptions = closeOptions;
                if (value.GetItemByName("CloseExtendLineOptions")?.Value is LineOptions closeExtendOptions)
                    this.CloseExtendLineOptions = closeExtendOptions;

                if (value.GetItemByName("MiddleLineOptions")?.Value is LineOptions middleOptions)
                    this.MiddleLineOptions = middleOptions;
                if (value.GetItemByName("MiddleExtendLineOptions")?.Value is LineOptions middleExtendOptions)
                    this.MiddleExtendLineOptions = middleExtendOptions;

                if (value.GetItemByName("ShowOpenLineLabel")?.Value is bool showOpenLabel)
                    this.ShowOpenLineLabel = showOpenLabel;

                if (value.GetItemByName("ShowHighLineLabel")?.Value is bool showHighLabel)
                    this.ShowHighLineLabel = showHighLabel;

                if (value.GetItemByName("ShowLowLineLabel")?.Value is bool showLowLabel)
                    this.ShowLowLineLabel = showLowLabel;

                if (value.GetItemByName("ShowCloseLineLabel")?.Value is bool showCloseLabel)
                    this.ShowCloseLineLabel = showCloseLabel;

                if (value.GetItemByName("ShowMiddleLineLabel")?.Value is bool showMiddleLabel)
                    this.ShowMiddleLineLabel = showMiddleLabel;

                if (value.GetItemByName("Font")?.Value is Font font)
        ...
        public override void OnPaintChart(PaintChartEventArgs args)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < this.DaysCount; i++)
                {
                    ...
                    if (leftX < args.Rectangle.Left - 10)
                        leftX = args.Rectangle.Left - 10;

                    float rightX = (float)currentWindow.CoordinatesConverter.GetChartX(range.EndDateTime) + halfBarWidth;
                    if (rightX > args.Rectangle.Right + 10)
                        rightX = args.Rectangle.Right + 10;
                        ...
                        if (highY > top && highY < bottom)
                        {
                            if (!isMainRangeOuside && this.HighLineOptions.Enabled)
                            {
                                ...
                                if (this.ShowHighLineLabel)
                                    this.DrawBillet(gr, range.High, ref rightX, ref highY, this.currentFont, this.highLineOptions, this.highLinePen, this.centerNearSF, "H:");
                            }
                        ...
                        if (lowY > top && lowY < bottom)
                        {
                            if (!isMainRangeOuside && this.LowLineOptions.Enabled)
                            {
                                ...
                                if (this.ShowLowLineLabel)
                                    this.DrawBillet(gr, range.Low, ref rightX, ref lowY, this.currentFont, this.lowLineOptions, this.lowLinePen, this.centerNearSF, "L:");
                            }
                        ...
                        if (openY > top && openY < bottom)
                        {
                            if (!isMainRangeOuside && this.OpenLineOptions.Enabled)
                            {
                                ...
                                if (this.ShowOpenLineLabel)
                                    this.DrawBillet(gr, range.Open, ref rightX, ref openY, this.currentFont, this.openLineOptions, this.openLinePen, this.centerNearSF, "O:");
                            }
                        ...
                        if (closeY > top && closeY < bottom)
                        {
                            if (!isMainRangeOuside && this.CloseLineOptions.Enabled)
                            {
                                ...
                                if (this.ShowCloseLineLabel)
                                    this.DrawBillet(gr, range.Close, ref rightX, ref closeY, this.currentFont, this.closeLineOptions, this.closeLinePen, this.centerNearSF, "C:");
                            }
                        ...
                        if (middleY > top && middleY < bottom)
                        {
                            if (!isMainRangeOuside && this.MiddleLineOptions.Enabled)
                            {
                                ...
                                if (this.ShowMiddleLineLabel)
                                    this.DrawBillet(gr, range.MiddlePrice, ref rightX, ref middleY, this.currentFont, this.middleLineOptions, this.middleLinePen, this.centerNearSF, "M:");
                            }
        ...
        private void CurrentChart_OnUpdateChartSettings()
        {
        ...
        private void DrawBillet(Graphics gr, double price, ref float rightX, ref float priceY, Font font, LineOptions lineOptions, Pen pen, StringFormat stringFormat, string text)
        {
            string label = text + this.Symbol.FormatPrice(price);
            var labelSize = gr.MeasureString(label, font);

            float posY = priceY - labelSize.Height - lineOptions.Width;

            var rect = new RectangleF()
            {
                Height = labelSize.Height,
                Width = labelSize.Width + 5,
                X = rightX - labelSize.Width - 5,
                Y = posY
            };
    ...
    public enum DailyOHLCSessionType
    {
        AllDay,
        SpecifiedSession,
        CustomRange,
