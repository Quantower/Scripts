...
public class IndicatorOpeningRange : Indicator, IWatchlistIndicator
{
	...
	[InputParameter("Show fill", 35)]
	public bool ShowFill = true;

	[InputParameter("Fill color", 36)]
    ...
    private const int SERIES_LOW = 1;
    private const int SERIES_EXT_U25 = 2;
    private const int SERIES_EXT_U50 = 3;
    private const int SERIES_EXT_U100 = 4;
    private const int SERIES_EXT_D25 = 5;
    private const int SERIES_EXT_D50 = 6;
    private const int SERIES_EXT_D100 = 7;
	...
	public override void OnPaintChart(PaintChartEventArgs args)
	{
		...
		try
		{
			...
			this.UpdateBrushes();
            if (this.ShowFill && this.LinesSeries[0].Visible && this.LinesSeries[1].Visible)
            {
                var wnd = this.CurrentChart.MainWindow;
                var cc = wnd.CoordinatesConverter;
                int halfBar = this.CurrentChart.BarsWidth / 2;

                float xStart = (float)cc.GetChartX(this.openingRange.LeftTime)  + halfBar;
                float xEnd = (float)cc.GetChartX(this.openingRange.RightTime) + halfBar;
            ...
            // draw 'Low' billet
            if (this.ShowLabels && this.LinesSeries[1].Visible)
				this.DrawBillet(gr, rightBarLowLineX,lowPriceY, this.LinesSeries[1].Width, $"L: {this.openingRange.LowPrice.FormattedValue}", this.lowLabelBrush, Brushes.White, this.font, LabelPosition.Bottom, this.centerNearSF);
        }
	...
	private void CalculateIndicator(DateTime itemTime, double highPrice, double lowPrice)
	{
		...
		if (hasTime)
		{
			if (this.openingRange.TryUpdate(highPrice, lowPrice, itemTime))
			{
                ...
                int endIndexWithinRange = (int)this.HistoricalData.GetIndexByTime(this.openingRange.RightTime.Ticks, SeekOriginHistory.Begin);
                if (endIndexWithinRange < 0 || endIndexWithinRange >= this.Count)
                    endIndexWithinRange = this.Count - 1;

                this.UpdateIndicatorLines(openingRange.HighPrice.Value, openingRange.LowPrice.Value, openingRange.StartIndex, endIndexWithinRange);
            }
        }
