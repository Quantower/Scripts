...
public class IndicatorHullMovingAverage : Indicator, IWatchlistIndicator
{
    ...
    protected override void OnInit()
    {
        ...
        this.wma = Core.Instance.Indicators.BuiltIn.LWMA((int)Math.Floor(Math.Sqrt(this.MaPeriod)), PriceType.Close);
    ...
    protected override void OnUpdate(UpdateArgs args)
    {
        if (this.Count < this.MaPeriod)
            return;
        double hullMWASourceValue = 2 * this.halfWMA.GetValue() - this.fullWMA.GetValue();
        this.hullWMASource.SetValue(0d, 0d, 0d, hullMWASourceValue);
