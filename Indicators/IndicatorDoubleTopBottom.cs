    ...
    public class IndicatorDoubleTopBottom : Indicator
    {
        ...
        public override void OnPaintChart(PaintChartEventArgs args)
        {
            ...
            try
            {
                ...
                //
                DateTime leftBorderTime = this.CurrentChart.MainWindow.CoordinatesConverter.GetTime(this.CurrentChart.MainWindow.ClientRectangle.Left);
                DateTime rightBorderTime = this.CurrentChart.MainWindow.CoordinatesConverter.GetTime(this.CurrentChart.MainWindow.ClientRectangle.Right);
                int leftIndex, rightIndex;
                if (Time() <= rightBorderTime)
                    rightIndex = 0 + smallPeriod;
                else
                {
                    int barIndex = (int)this.CurrentChart.MainWindow.CoordinatesConverter.GetBarIndex(rightBorderTime);
                    if (Count - barIndex - bigPeriod * 3 >= smallPeriod)
                        rightIndex = Count - barIndex;
                    else
                        rightIndex = 0 + smallPeriod;
                }
                if (Time(Count - 1) >= leftBorderTime)
                    leftIndex = Count - 1 - smallPeriod;
                else
                {
                    int barIndex = (int)this.CurrentChart.MainWindow.CoordinatesConverter.GetBarIndex(leftBorderTime);
                    if (Count - barIndex + bigPeriod * 2 <= Count - 1 - smallPeriod - bigPeriod * 2)
                        leftIndex = Count - barIndex;
                    else
                        leftIndex = Count - 1 - smallPeriod;
                }
                ...
                {
                    MaxOrMin maxOrMin = LocalMaxOrMin(i);
                ...
                if (pivots.Count <= 2)
                    return;
                double differenceInTicks = difference * Symbol.TickSize;
                for (int i = 0; i < pivots.Count - 2; i++)
                {
                    HistoryItemBar currentBar = (HistoryItemBar)this.HistoricalData[pivots[i].index];
                    HistoryItemBar nextBar = (HistoryItemBar)this.HistoricalData[pivots[i + 2].index];
                    int halfBarWidth = this.CurrentChart.BarsWidth / 2;
                    if (pivots[i].MaxOrMin == MaxOrMin.LocalMaximum && pivots[i + 1].MaxOrMin == MaxOrMin.LocalMinimum && pivots[i + 2].MaxOrMin == MaxOrMin.LocalMaximum && pivots[i + 2].index - pivots[i].index <= bigPeriod)
                    {
                        ...
                        {
                            Point firstPoint = new Point((int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartX(this.HistoricalData[pivots[i].index].TimeLeft) + halfBarWidth), (int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartY(currentHigh)));
                            Point secondPoint = new Point((int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartX(this.HistoricalData[pivots[i + 2].index].TimeLeft) + halfBarWidth), (int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartY(nexttHigh)));
                            graphics.DrawLine(doubleTopPen, firstPoint, secondPoint);
                            i = i + 2;
                        }
                    }
                    else if (pivots[i].MaxOrMin == MaxOrMin.LocalMinimum && pivots[i + 1].MaxOrMin == MaxOrMin.LocalMaximum && pivots[i + 2].MaxOrMin == MaxOrMin.LocalMinimum && pivots[i + 2].index - pivots[i].index <= bigPeriod)
                    {
                        ...
                        {
                            Point firstPoint = new Point((int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartX(this.HistoricalData[pivots[i].index].TimeLeft) + halfBarWidth), (int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartY(currentLow)));
                            Point secondPoint = new Point((int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartX(this.HistoricalData[pivots[i + 2].index].TimeLeft) + halfBarWidth), (int)(this.CurrentChart.MainWindow.CoordinatesConverter.GetChartY(nexttLow)));
                            graphics.DrawLine(doubleBottomPen, firstPoint, secondPoint);
                            i = i + 2;
                        }
                    }
                }
        ...
        private MaxOrMin LocalMaxOrMin(int index = 0)
        {
            ...
            for (int i = 1; i <= smallPeriod; i++)
            {
                HistoryItemBar leftBar = (HistoryItemBar)this.HistoricalData[index + i];
                HistoryItemBar rightBar = (HistoryItemBar)this.HistoricalData[index - i];
                HistoryItemBar currentBar = (HistoryItemBar)this.HistoricalData[index];
